...
export function GiftCard({
  ...
  return (
    <article className="overflow-hidden rounded-2xl border border-ink/10 bg-white shadow-sm">
      {publicMode ? image : <Link href={detailPath}>{image}</Link>}
      <div className="grid gap-3 p-3">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <p className="text-xs font-black uppercase text-berry">{gift.eventTag}</p>
            {publicMode ? (
              <h3 className="text-base font-black leading-tight">{gift.title}</h3>
            ) : (
              <Link className="block text-base font-black leading-tight hover:text-berry" href={detailPath}>
                {gift.title}
              </Link>
            )}
            <p className="mt-1 text-xs font-bold text-ink/55">
              {gift.storeName} · {priceLabel}
            </p>
          </div>
        ...
        {!publicMode ? (
          <div className="flex flex-wrap gap-2 text-xs font-black">
            <span className="rounded-full bg-blush px-2 py-1 text-berry">{gift.visibility}</span>
            {gift.hiddenFromRecipient ? (
              <span className="inline-flex items-center gap-1 rounded-full bg-cloud px-2 py-1 text-ink/70">
                <Lock size={12} />
                hidden
              </span>
            ) : null}
            {gift.reservedStatus === "reserved" ? <span className="rounded-full bg-mint px-2 py-1 text-spruce">reserved</span> : null}
            {gift.purchasedStatus ? <span className="rounded-full bg-honey/40 px-2 py-1">purchased</span> : null}
          </div>
        ) : (
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          {publicMode ? (
            <>
              <a
                className="focus-ring inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl bg-coral px-4 py-2 text-sm font-extrabold text-white hover:bg-berry"
              ...
              {gift.reservedStatus === "reserved" ? (
                ...
                <Button type="button" variant="secondary" onClick={() => onReserve?.(gift)}>
                  Reserve Gift
                </Button>
              )}
              {gift.allowContributions ? (
